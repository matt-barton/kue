...
Worker.prototype.start = function( fn, pre ) {
  var self = this;
  self.idle();
  ...
  self.getJob(function( err, job ) {
    ...
    var skip = (job && job.data && job.data.restrictToNamedWorker && self.queue.name && self.queue.name !== job.data.restrictToNamedWorker);
    if (!skip && job && 'function' === typeof self.pre) {
      skip = !self.pre(job);
    }
    if (skip) { job.reattempt(); }
    if( !job || skip || err ) return process.nextTick(function() {
      self.start(fn, self.pre);
    });
    self.process(job, fn);
  });
  return this;
